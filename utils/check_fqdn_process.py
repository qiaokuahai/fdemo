...
def single_process(sep_fqdns, df_ldns, out_file_name, per_cpu_task_count):
    ...
    for one_fqdns in thread_fqdns_lst:
        t = threading.Thread(target=to_dig, args=(one_fqdns, df_ldns, 2.0, out_file_name))
        t.start()
...
def task_start(fqdn_path, ldns_path, per_cpu_task_count):
    ...
    out_file_name = "result_boardcast_" + str(datetime.datetime.now())[:19].replace(" ", "_") + ".csv"
    df_ldns.set_index(keys=["ip_address"], drop=False, inplace=True)
    ...
    for sep_fqdns in sep_fqdns_lst:
        if sep_fqdns:
            p = Process(target=single_process,args=(sep_fqdns, df_ldns, out_file_name, per_cpu_task_count))
            p.start()
